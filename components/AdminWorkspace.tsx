...
export function AdminWorkspace({profile,jobs:initialJobs,applications:initialApps}:{profile:Profile;jobs:Job[];applications:Application[]}) {
  ...
  return <main className="admin-shell">
    <aside><div className="brand"><img src="/assets/logo.png" alt="YMX" className="logo-img"/><span><strong>Recruitment</strong><small>{profile.role} workspace</small></span></div><nav><button className={tab==="overview"?"active":""} onClick={()=>setTab("overview")}>Overview</button><button className={tab==="jobs"?"active":""} onClick={()=>setTab("jobs")}>Job postings</button><button className={tab==="candidates"?"active":""} onClick={()=>setTab("candidates")}>Candidates</button></nav></aside>
